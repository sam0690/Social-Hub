...
const SettingsSidebar = ({ activeSection, onSectionChange, search, onSearchChange, items }: SettingsSidebarProps) => {
  ...
  return (
    <div className="sticky top-16 flex h-[calc(100vh-4rem)] w-full flex-col gap-2 overflow-y-auto border-r border-slate-300 bg-white px-2 py-4 dark:border-white/10 dark:bg-black/50 lg:flex scrollbar-hide item-center ">
      <div className="flex items-center px-3 cursor-pointer">
       ...
       <div className="gap-2 rounded-full border border-slate-300 px-2 m-4 dark:border-white/10">
        <div className="flex items-center gap-2 py-1.5 text-slate-400 dark:text-white cursor-pointer">
          <Search size={14} />
        <input
          type="text"
          value={search}
          onChange={(event) => onSearchChange(event.target.value)}
          className="shrink-0 bg-transparent text-sm outline-none"
          placeholder='Search'
        />
        </div>
      </div>
