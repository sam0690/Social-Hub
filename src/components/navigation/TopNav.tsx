...
import { Bell, Search, Sparkles, User, X, CheckCheck, Clock3, Filter, Send, LogOut, RefreshCcw, Settings } from "lucide-react";
...
export default function TopNav() {
  ...
  return (
    <>
      <header className="fixed inset-x-0 top-0 z-40 border-b border-slate-200/70 bg-white/80 text-slate-900 backdrop-blur-xl dark:border-white/8 dark:bg-black/55 dark:text-white">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-3 px-4 sm:px-6 lg:px-8">
                        ...
                        <>
                          ...
                          <div className="flex flex-col gap-2">
                            {recentSearches.map((item) => (
                              <div key={item} className="flex items-center justify-between bg-white px-3 py-1.5 text-xs text-slate-700 transition hover:border-indigo-400/60 hover:bg-indigo-500/10 dark:border-white/8 dark:bg-white/5 dark:text-zinc-200">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button onClick={() => router.push("/settings")} className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-slate-200 bg-white/70 text-slate-700 transition hover:border-slate-300 hover:bg-white dark:border-white/10 dark:bg-white/5 dark:text-zinc-200 dark:hover:border-white/20 dark:hover:bg-white/10">
              <Settings size={17} />
            </button>

            <motion.div animate={{ rotate: rotation }} transition={{ duration: 1 }}>
              <button
        ...
        <AnimatePresence>
          {mobileSearchOpen && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed inset-0 z-60 bg-white/70 p-4 backdrop-blur-xl dark:bg-black/75 md:hidden"
            >
              <motion.div
                initial={{ y: 16, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                exit={{ y: 16, opacity: 0 }}
                className="mx-auto mt-12 max-w-md rounded-3xl border border-slate-300 bg-white p-4 shadow-2xl shadow-black/60 dark:border-white/10 dark:bg-[#0b0c16]"
              >
                ...
                <div className="mt-4 rounded-2xl border border-slate-300 bg-white/5 px-3 py-3 dark:border-white/8">
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      className="w-full bg-transparent text-sm text-slate-900 outline-none placeholder:text-zinc-500 dark:text-white"
                    />
                  </div>
                </div>
                ...
                <div className="mt-4 space-y-2">
                  {debouncedQuery ? (
                    searchData && searchData.length > 0 ? (
                      ...
                      <div className="py-4 text-center text-sm text-slate-500 dark:text-zinc-400">No result found</div>
                    )
                  ) : (
                    <>
                      <div className="text-xs uppercase tracking-[0.25em] text-zinc-500">Recent</div>
                      {recentSearches.map((item) => (
                        <button key={item} type="button" className="flex w-full items-center justify-between rounded-2xl border border-slate-300 bg-white/4 px-3 py-3 text-left text-sm text-slate-900 dark:border-white/8 dark:text-zinc-200">
                          <span>{item}</span>
